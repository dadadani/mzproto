...
const Primality = @import("../crypto/miller_rabin.zig");
...
fn validateDhPrime(io: std.Io, p: u2048, g: u32) std.Io.RandomSecureError!bool {
    ...
    if (!try Primality.isProbablyPrime2048(io, p, Primality.default_rounds)) return false;
    if (!try Primality.isProbablyPrime2048(io, (p - 1) / 2, Primality.default_rounds)) return false;
...
const AuthGen = struct {
    ...
    fn reqDH(self: *AuthGen, resPQ: *const tl.ProtoResPQ) !void {
        ...
        if (!try validateDhPrime(self.io, self.dh_prime, dhInnerData.g)) {
